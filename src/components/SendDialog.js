...
var addrarr = ""
export function addressfailed(address) {
  // console.log(address)
...
function SendSplDialog({ onClose, publicKey, balanceInfo, onSubmitRef }) {
  ...
  async function makeTransactionDua(address) {
    let amount = Math.round(parseFloat(transferAmountString) * 10 ** decimals);
    if (!amount || amount <= 0) {
      throw new Error('Invalid amount');
    }
  ...
  function SplitAddress(event){
    const csv = event.files[0]
    reader.readAsText(csv)
  }

  reader.onload = async function (e) {
    const arr_addr = String(e.target.result).replace(/\n|\r/g, "").split(',')
    beforeSubmit(arr_addr)
  }
  async function beforeSubmit(arr_addr){
    for (let address of arr_addr) {
      const ada = address.replace('"','').replace(/\n|\r/g, "").replace(' ',"");
      await onSubmitdua(`${String(ada)}`)
      await timeout(10000);
    }
  ...
  async function onSubmitdua(address) {
    return sendTransaction(makeTransactionDua(address), { onSuccess: onClose, onError: (e)=>addressfailed(`${address}\n`) });
  }
  ...
  return (
    <>
      ...
      <DialogActions>
        ...
        NOTE: .CSV ONLY <input type="file" name="file" accept=".csv" onChange={(e)=> SplitAddress(e.target)} />
